...
Set Up API Keys:
To access wind and tide data, you need to have API keys for the respective APIs (e.g., OpenWeather API for weather data).

Create a .env file in the root directory of the project.

Add your API keys (like OPENWEATHER_API_KEY for OpenWeather API) to the .env file:

OPENWEATHER_API_KEY=your-api-key-here


Run the Application:
Once everything is set up, you can start using the application. If you're working with a Jupyter notebook (main.ipynb), run the notebook directly:
...
The system will fetch wind and tide forecasts from public APIs.

This data is then processed to calculate the mooring line tension needed for each hook.
...
requirements.txt: Lists the Python libraries required to run the project (requests, beautifulsoup4, python-dotenv, etc.).
